var users=[{id:1 ,name:"sharad",email:"ksharad34@gmail.com",password:"Abhishek@123"}];


let doc=document.getElementById("name");
let em=document.getElementById("emsg")
let dlast=document.getElementById("last");
...
function Emailcheck(data)
 {
     if(!data.includes('@'))
     {
         em.innerHTML="Please write correct Email, Use @";ecount=0;
     }
     else
     {
         em.innerHTML="";
...
function Namecheck(data) {
    ...
    {
       // console.log("")
      doc.innerHTML=`Name must have 2 characters`;
      doc.style.color="red";
      ncount=0;
    }
    else{
...
var lcount;
function LastNamecheck(data) {
    if(data.length<2)
    {
       // console.log("")
      dlast.innerHTML=`Name must have 2 characters`;
      dlast.style.color="red";
      lcount=0;
...
var count=0;
 ...
 function Passcheck(data)
 {
  ...
  if(named.value==data)
  {
    message.innerText="Password must be different from Email and Name";
   ...
   if(fir==1 && sec==1 && thir==1 && four==1 && passame==1)
{
 ...
 function concheck(data)
 {
     if(pass.value!=data)
     {
         conmsg.innerHTML=`Confirm Password is not matching with Password `;
...
if(count==5 && ncount==1 && ecount==1 && concount==1 && lcount==1)
{ 
    
    
    
 sum=0;
 lastmsg.innerHTML=`Registered Successfully!`;
